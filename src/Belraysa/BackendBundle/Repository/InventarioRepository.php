...
class InventarioRepository extends EntityRepository
{
    ...
    public function findBusquedaSimple($text, $ws)
    {
        ...
        $query = $em->createQuery('SELECT c FROM BackendBundle:Inventario c WHERE c.workspace = :ws AND (c.nombre LIKE :query OR c.nombre LIKE :query1 OR c.nombre LIKE :query2) ORDER BY c.nombre ASC');
